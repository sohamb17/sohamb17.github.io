...
type LeetCodeStats = {
  ...
  username: string; solved: number; topPercentage: number | null; updatedAt: string;

  badge?: { name: string; icon: string | null } | null;

  rating?: number | null; contestCount?: number | null;

  history?: { rating: number; title: string; startTime: number }[];
...
function ContestBadge({ stats }: { stats: LeetCodeStats }) {
  ...
  return <span className="leetcode-badge-wrap"
    ...
    <button type="button" className="leetcode-contest-badge"
      ...
      aria-label={`LeetCode ${badge.name} contest details`}
    ...
    <span id="leetcode-contest-details" role="tooltip" className="leetcode-tooltip" hidden={!open}>

      <strong>{badge.name}</strong>
      {stats.rating != null && <span>Rating: {Math.round(stats.rating)}</span>}
    </span>
...
function LeetCodeCaption() {
  ...
  const stats = savedLeetCodeStats as LeetCodeStats | null;
